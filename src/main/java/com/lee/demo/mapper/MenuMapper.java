...
//    @Update("update menu set menu_desc=#{menuDesc}, update_time=#{updateTime}, name=#{name}, img_url=#{imageUrl} where menu_id=#{menuId}")
    ...
    @Select("select * from menu where user_id = #{userId} where name like '%#{keyword}%' or menu_desc like '%#{keyword}%'")
    List<MenuResponse> searchKeyword(String userId, String keyword);
